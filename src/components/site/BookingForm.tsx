...
import { addBooking, isTimeSlotLocked } from "@/lib/booking-store";
...
Booking ID: ${result.booking?.id || 'Pending'}`;
      ...
      const phoneNumber = "919865750184";
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select required value={formData.timeSlot} onValueChange={(value) => setFormData({ ...formData, timeSlot: value })}>
              ...
              <SelectContent>
                {TIME_SLOTS.map((item) => {
                  // Only check if locked if date is selected
                  let isLocked = false;
                  if (formData.date) {
                    try {
                      const timePart = item.slot.split('-')[0];
                      const timeStr = timePart.includes('PM') ? timePart.replace('PM', '').trim() : timePart;
                      const hour = parseInt(timeStr);
                      const isPM = item.slot.includes('PM');
                      const adjustedHour = isPM && hour < 12 ? hour + 12 : hour;
                      const dateObj = new Date(`${formData.date}T${adjustedHour.toString().padStart(2, '0')}:00:00`);
                      if (!isNaN(dateObj.getTime())) {
                        const datetime = dateObj.toISOString();
                        // Note: isTimeSlotLocked is async but we're using it synchronously here
                        // For now, we'll skip the async check in rendering to avoid complexity
                        // The actual check happens on form submit
                        isLocked = false;
                      }
                    } catch (error) {
                  ...
                  return (
                    <SelectItem 
                      ...
                      value={item.slot}
                      disabled={isLocked}
                      className={isLocked ? "text-red-500 font-semibold" : ""}
                    >
                      <div className="flex items-center justify-between w-full">
                        <span>{item.slot}</span>
                        <div className="flex items-center gap-2">
                          {isLocked && <span className="text-xs bg-red-100 text-red-600 px-2 py-1 rounded">Booked</span>}
                          <span className="text-xs text-muted-foreground">({item.batch.charAt(0).toUpperCase() + item.batch.slice(1)})</span>
                        </div>
                      </div>
                    </SelectItem>
                  );
